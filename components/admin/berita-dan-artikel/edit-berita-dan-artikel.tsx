...
export default function EditBeritaArtikel() {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        ...
        // ✅ Simpan authorId dari data artikel yang ada
  ...
  const handleSave = async (newStatus?: "DRAFT" | "PUBLISHED") => {
    ...
    // ✅ Gunakan authorId dari artikel, fallback ke googleUser.id
    const finalAuthorId = authorId ?? googleUser?.id;
    if (!finalAuthorId) {
      setError("Author tidak ditemukan, silakan login ulang."); return;
    }
    ...
    try {
      ...
      formData.append("authorId", String(finalAuthorId)); // ✅ authorId disertakan
    ...
    <div className="min-h-screen bg-[#e8f5e9] px-6 py-8 flex flex-col gap-5">
      ...
      <div className="flex justify-end gap-4 mt-2">
        ...
        <button onClick={() => handleSave("DRAFT")} disabled={saving}
          className="px-8 py-2.5 rounded-full text-sm font-semibold text-gray-600 bg-white hover:bg-gray-100 transition border border-gray-200 disabled:opacity-50 flex items-center gap-2">
          {saving && <div className="animate-spin w-4 h-4 border-2 border-gray-400 border-t-transparent rounded-full" />}
          {saving ? "Menyimpan..." : "Perbarui Draft"}
        </button>
