...
export default function AddBeritaArtikel() {
  ...
  const handleGambarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) { setGambar(file); setPreview(URL.createObjectURL(file)); }
  };

  const handleSubmit = async (status: "DRAFT" | "PUBLISHED") => {
    if (!form.title.trim() || !form.content.trim()) {
      setError("Judul dan konten wajib diisi!"); return;
    }

    const token = getAccessToken();
    if (!token) {
      setError("Sesi habis, silakan login ulang."); return;
    }
    if (form.content.trim().length < 10) {
    setError("Konten minimal 10 karakter!"); return;
    }
    if (form.summary.trim().length > 0 && form.summary.trim().length < 5) {
      setError("Ringkasan minimal 5 karakter jika diisi!"); return;
    }

    const authorId = googleUser?.id;
    if (!authorId) {
      setError("User tidak ditemukan, silakan login ulang."); return;
    }
    ...
    try {
      ...
      formData.append("authorId", String(authorId)); // ✅ dari user yang login
  ...
  return (
    <div className="min-h-screen bg-[#e8f5e9] px-6 py-8 flex flex-col gap-5">
      ...
      <div className="flex justify-end gap-4 mt-2">
        <button onClick={() => router.push("/admin/berita-dan-artikel")} disabled={loading}
        ...
        <button onClick={() => handleSubmit("DRAFT")} disabled={loading}
          className="px-8 py-2.5 rounded-full text-sm font-semibold text-gray-600 bg-white hover:bg-gray-100 transition border border-gray-200 shadow-sm disabled:opacity-50 flex items-center gap-2">
          {loading && <div className="animate-spin w-4 h-4 border-2 border-gray-400 border-t-transparent rounded-full" />}
